...
export function listRoomTypeReviews(roomTypeId) {
  return findPublicRoomTypeReviews(roomTypeId);
}
export function listGuestReviews(userId) {
...
export async function createGuestReview(userId, bookingId, data) {
  ...
  const errors = {
    ...
    STAY_NOT_COMPLETED: [
      409,
      'You can review this room after completing your stay',
    ],
    PAYMENT_REQUIRED: [
      409,
      'A successful payment is required before reviewing this stay',
    ],
...
export async function updateGuestReview(id, userId, data) {
  if (!(await updateReviewRecord(id, userId, data)))
    throw new AppError(404, 'Review not found');
  return findReviewById(id);
}
export async function deleteGuestReview(id, userId) {
  if (!(await deleteReviewRecord(id, userId)))
    throw new AppError(404, 'Review not found');
}
