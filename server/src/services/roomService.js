...
function translateDatabaseError(error) {
  if (error.code === 'ER_DUP_ENTRY') {
    throw new AppError(
      409,
      'A room or room type with this value already exists',
    );
  }
  ...
  if (error.code === 'ER_ROW_IS_REFERENCED_2') {
    throw new AppError(
      409,
      'Delete the rooms assigned to this room type first',
    );
  }
  throw error;
}
...
export async function removeRoomType(id) {
  try {
    if (!(await deleteRoomTypeById(id)))
...
export async function addRoomTypeImages(roomTypeId, files) {
  ...
  const uniqueFiles = files
    ...
    .filter(({ contentHash }) => {
      if (existingHashes.has(contentHash) || requestHashes.has(contentHash))
        return false;
  ...
  await insertRoomTypeImages(
    roomTypeId,
    uniqueFiles.map(({ file, contentHash }) => ({
