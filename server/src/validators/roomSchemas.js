...
export const publicRoomSearchSchema = z.object({
  ...
  query: z.object({
    ...
    .refine(
      (query) => Boolean(query.checkIn) === Boolean(query.checkOut),
      { message: 'Check-in and check-out dates must be supplied together' },
    )
    .refine(
      (query) => !query.checkIn || !query.checkOut || query.checkOut > query.checkIn,
...
const roomTypeBody = z.object({
  ...
  imageUrl: z.union([
    z.url().max(1000),
    z.string().regex(/^\/rooms\/[A-Za-z0-9._/-]+$/, 'Use an HTTPS URL or a /rooms/... local path').max(1000),
    z.literal(''),
  ]).optional(),
}).strict();
