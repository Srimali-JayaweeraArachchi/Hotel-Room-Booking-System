...
try {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      console.log(
        `Migrated image ${row.id} to MySQL (${imageData.length} bytes)`,
      );
    } catch (error) {
      if (error.code === 'ENOENT')
        console.warn(
          `Skipped missing local file for image ${row.id}: ${filename}`,
        );
      else throw error;
    }
  }
  ...
  for (const row of blobRows) {
    const contentHash = crypto
      .createHash('sha256')
      .update(row.imageData)
      .digest('hex');
    const seen = seenByRoomType.get(row.roomTypeId) ?? new Set();
    if (seen.has(contentHash)) {
      await database.execute('DELETE FROM room_type_images WHERE id = ?', [
        row.id,
      ]);
    ...
    await database.execute(
      'UPDATE room_type_images SET content_hash = ? WHERE id = ?',
      [contentHash, row.id],
    );
  }
} finally {
