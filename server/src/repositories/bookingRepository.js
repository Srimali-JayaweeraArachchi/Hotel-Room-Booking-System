...
export async function createBookingRecord(data) {
  ...
  try {
    ...
    if (!roomType) { await connection.rollback(); return { reason: 'ROOM_TYPE_NOT_FOUND' }; }
    if (Number(roomType.capacity) < data.guests) { await connection.rollback(); return { reason: 'CAPACITY_EXCEEDED' }; }
    ...
    if (!roomRows[0]) { await connection.rollback(); return { reason: 'NO_AVAILABILITY' }; }
    ...
    const [result] = await connection.execute(
      `INSERT INTO bookings
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [reference, data.userId, roomRows[0].id, data.checkIn, data.checkInTime, data.checkOut, data.checkOutTime, data.guests, totalAmount, data.specialRequests || null],
    );
    ...
    await createRoleNotificationRecords(connection, {
      bookingId: result.insertId, eventType: 'booking_created', title: 'New booking received',
...
export async function findBookingById(id) {
  const [rows] = await database.execute(`${bookingSelect} WHERE b.id = ? LIMIT 1`, [id]);
...
export async function findAllBookings(filters = {}) {
  ...
  if (filters.search) {
    conditions.push('(b.reference LIKE ? OR u.name LIKE ? OR u.email LIKE ? OR r.room_number LIKE ?)');
    const term = `%${filters.search}%`;
    values.push(term, term, term, term);
  }
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
  const [rows] = await database.execute(`${bookingSelect} ${where} ORDER BY b.created_at DESC`, values);
  return rows.map(normalizeBooking);
}
...
export async function updateGuestBookingRecord(id, userId, data) {
  ...
  try {
    ...
    if (!booking) { await connection.rollback(); return { reason: 'NOT_FOUND' }; }
    if (booking.status !== 'pending') { await connection.rollback(); return { reason: 'NOT_MODIFIABLE' }; }
    if (Number(booking.capacity) < data.guests) { await connection.rollback(); return { reason: 'CAPACITY_EXCEEDED' }; }
    ...
    if (overlaps[0]) { await connection.rollback(); return { reason: 'NO_AVAILABILITY' }; }

    await connection.execute(
      `UPDATE bookings SET check_in = ?, check_in_time = ?, check_out = ?, check_out_time = ?, guests = ?,
       total_amount = ?, special_requests = ? WHERE id = ?`,
      [data.checkIn, data.checkInTime, data.checkOut, data.checkOutTime, data.guests, Number(booking.basePrice) * data.nights, data.specialRequests || null, id],
    );
    await connection.commit();
    return { bookingId: id };
  } catch (error) {
...
export async function cancelGuestBookingRecord(id, userId) {
  ...
  try {
    await connection.beginTransaction();
    const [rows] = await connection.execute('SELECT status FROM bookings WHERE id = ? AND user_id = ? FOR UPDATE', [id, userId]);
    if (!rows[0]) { await connection.rollback(); return { reason: 'NOT_FOUND' }; }
    if (!['pending', 'confirmed'].includes(rows[0].status)) { await connection.rollback(); return { reason: 'NOT_CANCELLABLE' }; }
    await connection.execute("UPDATE bookings SET status = 'cancelled', cancelled_at = CURRENT_TIMESTAMP WHERE id = ?", [id]);
    ...
    await createNotificationRecord(connection, {
      userId, bookingId: id,
      eventType: rows[0].status === 'confirmed' ? 'booking_refunded' : 'booking_cancelled',
      title: rows[0].status === 'confirmed' ? 'Booking cancelled and refunded' : 'Booking cancelled',
      message: rows[0].status === 'confirmed'
    ...
    await createRoleNotificationRecords(connection, {
      bookingId: id,
      eventType: rows[0].status === 'confirmed' ? 'booking_refunded' : 'booking_cancelled',
      title: rows[0].status === 'confirmed' ? 'Guest cancellation and refund' : 'Guest cancelled booking',
...
export async function updateBookingStatusRecord(id, status, changedBy, note) {
  ...
  try {
    await connection.beginTransaction();
    const [rows] = await connection.execute('SELECT status, user_id AS userId, reference FROM bookings WHERE id = ? FOR UPDATE', [id]);
    if (!rows[0]) { await connection.rollback(); return { reason: 'NOT_FOUND' }; }
    ...
    if (status === 'cancelled') {
      ...
      if (paidRows[0]) { await connection.rollback(); return { reason: 'PAID_BOOKING' }; }
    }
    await connection.execute(
    ...
    const messages = {
      confirmed: ['Booking confirmed', `Reservation ${rows[0].reference} was confirmed by the front desk.`],
      cancelled: ['Booking cancelled', `Reservation ${rows[0].reference} was cancelled by the front desk.`],
      checked_in: ['Check-in completed', `You have been checked in for reservation ${rows[0].reference}.`],
      completed: ['Stay completed', `Check-out is complete for reservation ${rows[0].reference}. Thank you for staying with us.`],
    };
    if (messages[status]) {
      await createNotificationRecord(connection, {
        userId: rows[0].userId, bookingId: id, eventType: `booking_${status}`,
        title: messages[status][0], message: messages[status][1],
      });
      if (['checked_in', 'completed', 'cancelled'].includes(status)) {
        await createRoleNotificationRecords(connection, {
          bookingId: id, eventType: `booking_${status}`,
