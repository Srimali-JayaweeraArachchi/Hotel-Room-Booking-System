...
async function attachImages(roomTypes) {
  ...
  for (const row of rows) {
    const image = { ...row, id: Number(row.id), roomTypeId: Number(row.roomTypeId), sortOrder: Number(row.sortOrder) };
...
export async function findRoomTypes(filters = {}) {
  ...
  if (filters.search) {
    conditions.push('(rt.name LIKE ? OR rt.description LIKE ? OR rt.bed_type LIKE ?)');
...
export async function insertRoomTypeImages(roomTypeId, images) {
  ...
  const placeholders = images.map((image, index) => {
    values.push(roomTypeId, image.imageData, image.mimeType, image.fileSize, image.contentHash, image.altText || null, index);
    return '(?, NULL, ?, ?, ?, ?, ?, ?)';
  });
...
export async function findRoomTypeImages(roomTypeId) {
  ...
  return rows.map((row) => ({ ...row, id: Number(row.id), roomTypeId: Number(row.roomTypeId), sortOrder: Number(row.sortOrder) }));
}
...
export async function findRoomTypeImageById(id, roomTypeId) {
  ...
  return rows[0] ? { ...rows[0], id: Number(rows[0].id), roomTypeId: Number(rows[0].roomTypeId) } : null;
}
...
export async function deleteRoomTypeImageById(id, roomTypeId) {
  const [result] = await database.execute('DELETE FROM room_type_images WHERE id = ? AND room_type_id = ?', [id, roomTypeId]);
...
export async function deleteRoomTypeById(id) {
  const [result] = await database.execute('DELETE FROM room_types WHERE id = ?', [id]);
  return result.affectedRows > 0;
}
...
export async function insertRoom(data) {
  const [result] = await database.execute(
    `INSERT INTO rooms (room_number, room_type_id, floor, status, notes)
     VALUES (?, ?, ?, ?, ?)`,
    [data.roomNumber, data.roomTypeId, data.floor, data.status, data.notes || null],
  );
...
export async function updateRoomById(id, data) {
  const [result] = await database.execute(
    `UPDATE rooms SET room_number = ?, room_type_id = ?, floor = ?, status = ?, notes = ?
     WHERE id = ?`,
    [data.roomNumber, data.roomTypeId, data.floor, data.status, data.notes || null, id],
  );
  return result.affectedRows ? findRoomById(id) : null;
}

export async function deleteRoomById(id) {
  const [result] = await database.execute('DELETE FROM rooms WHERE id = ?', [id]);
  return result.affectedRows > 0;
}
