...
function normalize(row) {
  return row ? { ...row, id: Number(row.id), bookingId: Number(row.bookingId), userId: Number(row.userId), roomTypeId: Number(row.roomTypeId), rating: Number(row.rating) } : null;
}
...
export async function findPublicRoomTypeReviews(roomTypeId) {
  ...
  const averageRating = reviews.length
    ...
    : 0;
  return { reviews, reviewCount: reviews.length, averageRating: Number(averageRating.toFixed(1)) };
}
...
export async function createReviewRecord(userId, bookingId, data) {
  ...
  try {
    ...
    if (booking.status !== 'completed') { await connection.rollback(); return { reason: 'STAY_NOT_COMPLETED' }; }
    const [payments] = await connection.execute(
      "SELECT id FROM payments WHERE booking_id = ? AND status = 'succeeded' LIMIT 1 FOR UPDATE",
      [bookingId],
    );
    if (!payments[0]) { await connection.rollback(); return { reason: 'PAYMENT_REQUIRED' }; }
    const [existing] = await connection.execute('SELECT id FROM reviews WHERE booking_id = ? LIMIT 1 FOR UPDATE', [bookingId]);
    if (existing[0]) { await connection.rollback(); return { reason: 'ALREADY_REVIEWED' }; }
    const [result] = await connection.execute(
      `INSERT INTO reviews (booking_id, user_id, room_type_id, rating, title, comment)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [bookingId, userId, booking.roomTypeId, data.rating, data.title, data.comment],
    );
    await createRoleNotificationRecords(connection, {
      bookingId, eventType: 'review_submitted', title: 'New guest review',
...
export async function findReviewById(id) {
  const [rows] = await database.execute(
    `SELECT ${reviewColumns} FROM reviews rv JOIN users u ON u.id = rv.user_id WHERE rv.id = ? LIMIT 1`, [id],
  );
...
export async function deleteReviewRecord(id, userId) {
  const [result] = await database.execute('DELETE FROM reviews WHERE id = ? AND user_id = ?', [id, userId]);
  return result.affectedRows > 0;
}
