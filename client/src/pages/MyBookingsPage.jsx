...
function MyBookingsPage() {
  ...
  const [payingId, setPayingId] = useState(
    location.state?.payBookingId ?? null,
  );
  ...
  async function saveEdit(event) {
    ...
    try {
      await updateBooking(editing.id, editing);
      setEditing(null);
      await refresh();
  ...
  return (
    <main className="bookings-page">
      <section className="rooms-heading">
        ...
        <h1>My bookings</h1>
        <p>
          Review upcoming stays, update reservation details, or cancel eligible
          bookings.
        </p>
      </section>
        ...
        {bookings.length ? (
          bookings.map((booking) => (
            <article className="booking-card" key={booking.id}>
              <div className="booking-reference">
                ...
                <div className="booking-statuses">
                  {booking.paymentStatus && (
                    <strong
                      className={`status-pill payment-${booking.paymentStatus}`}
                    >
                      payment {booking.paymentStatus}
                    </strong>
                  )}
                  <strong className={`status-pill status-${booking.status}`}>
              ...
              {payingId === booking.id && booking.status === 'pending' ? (
                <PaymentForm
                  booking={booking}
                  onCancel={() => setPayingId(null)}
                  onPaid={paymentComplete}
                />
              ) : editing?.id === booking.id ? (
                <form className="booking-edit" onSubmit={saveEdit}>
                  <div className="form-row">
                    ...
                    <label>
                      ...
                      <input
                        ...
                        name="guests"
                        onChange={(e) =>
                          setEditing({ ...editing, guests: e.target.value })
                        }
                        required
                        type="number"
                ...
                (reviewingId === booking.id ? (
                  ...
                  <div className="submitted-review">
                    <div>
                      ...
                      <strong>{booking.reviewTitle}</strong>
                    ...
                    <div>
                      ...
                      <button
                        className="table-action danger"
                        onClick={() => removeReview(booking.reviewId)}
                        type="button"
                      >
                        Delete review
                      </button>
          ...
          <div className="empty-state">
            ...
            <p>Your reservations will appear here after you select a room.</p>
          </div>
        )}
      </div>
    </main>
