  ...
  return (
    <main className="my-reviews-page">
      <section className="rooms-heading">
        ...
        <h1>My reviews</h1>
        <p>View every verified-stay review you have published.</p>
      </section>
      {error && (
      ...
      <section className="my-review-list">
        {reviews.map((review) => (
          <article className="my-review-card" key={review.id}>
            ...
            <footer>
              <div>
                <strong>{review.roomTypeName}</strong>
                <small>Booking {review.bookingReference}</small>
              </div>
              <Link className="text-link" to={`/rooms/${review.roomTypeId}`}>
                View room →
              </Link>
            </footer>
          </article>
        ...
        {!reviews.length && !error && (
          <div className="empty-state">
            ...
            <p>
              After a paid stay is completed, you can publish a review from My
              Bookings.
            </p>
